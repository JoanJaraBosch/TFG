...
echo "Generating key request for $domain"
openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/nginx/ssl/nginx_pub.key -subj "/C=$country/ST=$state/L=$locality/O=$organization/OU=$organizationalunit/CN=$commonname/emailAddress=$email" -out /etc/nginx/ssl/nginx_pub.crt

echo "---------------------------"
echo "-----Below is your CRT-----"
echo "---------------------------"
echo
cat /etc/nginx/ssl/nginx_pub.crt
...
echo "---------------------------"
echo
cat /etc/nginx/ssl/nginx_pub.key

openssl dhparam -out /etc/nginx/ssl/dhparam_pub.pem 4096
